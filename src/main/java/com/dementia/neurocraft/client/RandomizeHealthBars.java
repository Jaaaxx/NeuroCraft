...
public class RandomizeHealthBars {
    static int c = 1;
    static boolean barsWereRandomized = false;
    ...
    public static void onPlayerTickEvent(TickEvent.PlayerTickEvent tick) {
        if (tick.side == LogicalSide.CLIENT && tick.phase == TickEvent.Phase.END) {
            if (c++ % 10 == 0 && RANDOMIZE_BARS || barsAreRandomized != barsWereRandomized) {
                if (barsAreRandomized != barsWereRandomized) {

                }
                var playerSanity = getPlayerSanityClient();
                boolean switchBars = new Random().nextInt(PEAK_SANITY) < playerSanity;
                if (switchBars) {
